#!/usr/bin/env python3
"""Does the James-Stein shrinkage constant calibrated on CLEAN, fully-
observed data (c=1.0, from sim_delta_init_mom_shrinkage.py's n_restarts=4
sweep) still hold under TP53-shaped missingness with the "partial"
projection method (sim_delta_init_missingness_projection.py), or does the
partial projection's heterogeneous per-row precision (some rows full-weight,
others heavily renormalized) mean the standard skewness-SE formula
under-estimates true noise there, requiring a different (probably higher)
c to get the same zero/small-regime false-positive control?

Same James-Stein c grid, same 4 skew regimes, same n_restarts=4 as
sim_delta_init_mom_shrinkage.py's n_restarts=4 run, for direct
side-by-side comparison against that script's clean-data numbers -- this
script only changes the DATA (TP53-shaped block missingness via
inject_block_missingness, DIRECTION concentrated on 2 dims each of blocks
A/B, p=16) and the projection mechanism (projection_method="partial"
throughout, instead of the clean-data "complete" gate).

Usage:
    python tests/cfusn_simulations/sim_delta_init_missingness_c_sweep.py
    python tests/cfusn_simulations/sim_delta_init_missingness_c_sweep.py --n-seeds 10
"""
import argparse
import csv
import sys
...
def _report(rows):
    print(f"\n{'═' * 100}")
    print(f"  PARTIAL-PROJECTION James-Stein c SWEEP under TP53-shaped missingness "
          f"(n_restarts={N_RESTARTS}) -- vs. clean-data c=1.0 baseline")
    print(f"{'═' * 100}")
    by_key = {}
    for r in rows:
...
def _write_csv(rows):
    ...
    path = RESULTS_DIR / f"sim_delta_init_missingness_c_sweep_{ts}.csv"
    if rows:
