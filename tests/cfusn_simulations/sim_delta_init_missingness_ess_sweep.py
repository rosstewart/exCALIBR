#!/usr/bin/env python3
"""Does correcting the z-score's standard-error formula to use an
EFFECTIVE sample size (Kish's n_eff, weighting each row by how much of the
eigenvector's weight was actually observed for it) instead of the raw row
count let a SINGLE James-Stein c value work well for BOTH clean, fully-
observed data AND TP53-shaped missing data -- rather than needing two
separately-calibrated constants (c=1.0 for clean, c=4.0-ish as an
imperfect compromise for missing, per sim_delta_init_missingness_c_sweep.py's
finding that plain-count z was inflated under missingness)?

Same James-Stein c grid, same 4 skew regimes, same TP53-shaped
inject_block_missingness setup, same n_restarts=4 as
sim_delta_init_missingness_c_sweep.py -- the ONLY change is
init_delta_matrix_mom_shrunk_projection_ess (effective-sample-size
weighted z) in place of init_delta_matrix_mom_shrunk_projection's plain
z-score, so this is a direct apples-to-apples comparison against that
script's numbers.

Usage:
    python tests/cfusn_simulations/sim_delta_init_missingness_ess_sweep.py
    python tests/cfusn_simulations/sim_delta_init_missingness_ess_sweep.py --n-seeds 10
"""
import argparse
import csv
import sys
...
from tests.cfusn_simulations.sim_utils import (
    sample_cfusn, inject_block_missingness, init_delta_matrix_mom_shrunk_projection_ess,
...
def _report(rows):
    print(f"\n{'═' * 100}")
    print(f"  EFFECTIVE-SAMPLE-SIZE-CORRECTED partial-projection James-Stein c SWEEP "
          f"under TP53-shaped missingness (n_restarts={N_RESTARTS})")
    print(f"{'═' * 100}")
    by_key = {}
    for r in rows:
...
def _write_csv(rows):
    ...
    path = RESULTS_DIR / f"sim_delta_init_missingness_ess_sweep_{ts}.csv"
    if rows:
